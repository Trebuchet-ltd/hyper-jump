...
import * as DefaultSystemEvents from "./primitive/event.js"; // import {
//     loadAudioSources,
//     updateAudioSources,
//     updateAudioNodes,
//     stereo,
...
window.model = 0; // If requested, use the polyfill to provide support for mobile devices
...
let time = 0; // WebGL scene globals.

let gl = null;
let renderer = null;
let radii = new Float32Array(25);
let positions = new Float32Array(16 * 25); // Boxes
...
global.scene().standingStats(true); // global.scene().addNode(window.models['stereo']);
...
function addBox(x, y, z, r, g, b, offset) {
  ...
  boxNode.addRenderPrimitive(boxRenderPrimitive); // Marks the node as one that needs to be checked when hit testing.
...
export function initXR() {
  ...
  window.EventBus = new EventBus(); // window.objs = [];

  DefaultSystemEvents.init(); // websocket
...
function initGL() {
  ...
  global.scene().setRenderer(renderer); // Loads a generic controller meshes.

  global.scene().inputRenderer.setControllerMesh(new Gltf2Node({
...
async function onSessionStarted(session) {
  ...
  session.addEventListener("select", ev => {
    let refSpace = ev.frame.session.isImmersive ? inputController.referenceSpace : inlineViewerHelper.referenceSpace;
    global.scene().handleSelect(ev.inputSource, ev.frame, refSpace);
  });
  initGL();
  initHands(); // scene.inputRenderer.useProfileControllerMeshes(session);
...
function onSessionEnded(event) {
  if (event.session.isImmersive) {
    xrButton.setSession(null); // Stop the audio playback when we exit XR.
    // pauseAudio();
  }
}
...
function updateInputSources(session, frame, refSpace) {
  for (let inputSource of session.inputSources) {
    ...
    let offset = 0; // We may not get a pose back in cases where the input source has lost
    ...
    let cursorPos = vec3.fromValues(targetRay.origin.x, targetRay.origin.y, targetRay.origin.z);
    vec3.add(cursorPos, cursorPos, [targetRay.direction.x * cursorDistance, targetRay.direction.y * cursorDistance, targetRay.direction.z * cursorDistance]); // vec3.transformMat4(cursorPos, cursorPos, inputPose.targetRay.transformMatrix);
    ...
    if (inputSource.hand) {
      ...
      for (const box of boxes[inputSource.handedness]) {
        ...
        offset++; // mat4.getTranslation(box.translation, matrix);
      ...
      if (gripPose) {
        ...
        global.scene().inputRenderer.addController(gripPose.transform.matrix, inputSource.handedness); // let controller = this._controllers[handedness]; // so it is updating actually
        // ZH: update location
        // if (window.playerid) {
    ...
    let headPose = frame.getViewerPose(refSpace); // if (window.playerid) {
    ...
    for (let source of session.inputSources) {
      if (!window.handtracking && source.handedness && source.gamepad) {
        ...
        // }
        if (source.handedness == "left") window.avatars[window.playerid].leftController.updateButtons(source.gamepad.buttons);
        if (source.handedness == "right") window.avatars[window.playerid].rightController.updateButtons(source.gamepad.buttons); // console.log("leftController", window.avatars[window.playerid].leftController);
...
function onXRFrame(t, frame) {
  ...
  let refSpace = session.isImmersive ? inputController.referenceSpace : inlineViewerHelper.referenceSpace;
  let pose = frame.getViewerPose(refSpace);
  session.requestAnimationFrame(onXRFrame);
  keyboardInput.updateKeyState();
  global.scene().startFrame();
  updateInputSources(session, frame, refSpace); // ZH: send to websocket server for self avatar sync
  ...
  updateAvatars();
  updateObjects(); // ZH: save previous "source.gamepad.buttons" for two controllers,
  ...
  global.scene().drawXRFrame(frame, pose, time); // if (pose) {
...
function updateObjects() {
  ...
  for (let id in window.objects) {
    ...
    let matrix = window.objects[id]["matrix"]; // create the model if model is null
