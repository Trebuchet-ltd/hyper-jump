...
if (typeof window === 'undefined') {
  throw new Error('This library is designed to run in a browser.');
} // test if namespace is already used
// eslint-disable-next-line no-undef


if (typeof window.corelink !== 'undefined') {
  throw new Error('The corelink namespace is already registered.');
} // Main Namespace
...
(function () {
  ...
  */
  ...
  control.connect = async (IP, port) => new Promise(async (resolve, reject) => {
    ...
    control.request = async data => {
      if (debug) console.log('request', data); // eslint-disable-next-line no-shadow
      ...
      return new Promise(async (resolve, reject) => {
        ...
        control.client.addEventListener('message', async content => {
          // it should be data instead of message (Abhishek Khanna), Rob : please review
          ...
          for (let i = 0; i < parsed.length; i += 1) {
            ...
            if ('ID' in parsed[i] && parsed[i].ID === wdata.ID) {
              ...
              if ('statusCode' in parsed[i]) {
                ...
                if ('message' in parsed[i]) reject(new Error(`${parsed[i].message} (${parsed[i].statusCode})`));else reject(new Error(`Error (${parsed[i].statusCode}) with out specific message returned.`));
              }
    ...
    */
    ...
    control.client.onclose = () => {
      ...
      if (connConfig.autoReconnect) {
        if (closeCb != null) closeCb();else console.log('No close connection callback provided.');
      ...
      } else if (closeCb != null) closeCb();else {
    ...
    return new Promise((resolve, reject) => {
      ...
      receiverStream.client.addEventListener('message', message => {
        if (dataCb != null) {
          ...
          const headSize = view.getUint16(0, true); // const dataSize = view.getUint16(2, true)
  ...
  */
  ...
  async function createSender(options) {
    return new Promise(async (resolve, reject) => {
      const workOptions = options; // checking inputs
      ...
      if ('streamID' in content) senderStreams[content.streamID] = [];else reject(new Error('StreamID not found.'));
      if ('port' in content) senderStreams[content.streamID].port = content.port;else reject(new Error('Target port not found.'));
      if ('MTU' in content) senderStreams[content.streamID].MTU = content.MTU;else reject(new Error('Target MTU not found.'));
      ...
      if (workOptions.protocol === 'ws') await setupSender(content.streamID, content.port).catch(err => {
        console.log(err);
      });
  ...
  */
  ...
  async function createReceiver(options) {
    return new Promise(async (resolve, reject) => {
      ...
      if ('receiverID' in workOptions) workOptions.receiverID = `,"receiverID":"${workOptions.receiverID}"`;else workOptions.receiverID = '';
      ...
      if ('streamID' in content) receiverStream.streamID = content.streamID;else reject(new Error('StreamID not found.'));
      if (debug) console.log(`createReceiver port: ${content.port}`);
      if ('port' in content) receiverStream.port = content.port;else reject(new Error('Target port not found.'));
      if ('proto' in content) receiverStream.proto = content.proto;else reject(new Error('Target proto not found.'));
      if ('streamList' in content) receiverStream.streamList = content.streamList;else reject(new Error('Target streamList not found.'));
      if ('MTU' in content) receiverStream.MTU = content.MTU;else reject(new Error('Target MTU not found.'));
  ...
  */
  ...
  function send(streamID, data, header) {
    if (typeof senderStreams[streamID] !== 'undefined' && senderStreams[streamID].protocol === 'ws') {
      let workingHeader;
      if (typeof header === 'object') workingHeader = JSON.stringify(header);else workingHeader = ''; // adding header
      ...
      const dataView = new Uint8Array(data); // add json header to send buffer
  ...
  async function login(credentials) {
    return new Promise(async (resolve, reject) => {
      ...
      if (typeof credentials.username === 'undefined' && typeof credentials.password === 'undefined') {
        if (typeof credentials.token === 'undefined') reject(new Error('Credentials not found.'));else request = `{"function":"auth","token":"${credentials.token}"}`;
      ...
      if ('token' in content) token = content.token;else reject(new Error('Token not found.'));
      if ('IP' in content) sourceIP = content.IP;else reject(new Error('SourceIP not found.'));
      resolve(true);
    });
  }
  /**
  ...
  async function connect(credentials, config) {
    return new Promise(async (resolve, reject) => {
      ...
      connConfig = config;
      connCredentials = credentials; // dns.lookup(connConfig.ControlIP, (err, address) => {
      ...
      let conn;
      connected = true; // connect the client
      ...
      if (connected) {
        ...
        if (token == null) conn = await login(credentials).catch(e => reject(e)); // if all streams are still valid
  ...
  async function setConfig(options) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!('config' in request)) {
        reject(new Error('Name of the configuration parameter required.'));
        return;
      }

      if (!('value' in request)) {
        reject(new Error('Context of the configuration parameter required.'));
        return;
      }

      if (!('context' in request) || request === '') {
  ...
  async function listUsers() {
    return new Promise(async (resolve, reject) => {
      ...
      const content = await control.request(JSON.parse(request)).catch(e => reject(e));
      if ('userList' in content) resolve(content.userList);else reject(new Error('Users not found.'));
    });
  ...
  async function addUserGroup(options) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!('username' in workOptions)) reject(new Error('user not found.'));else if (!('group' in workOptions)) reject(new Error('group not found.'));else {
  ...
  async function listGroups() {
    return new Promise(async (resolve, reject) => {
      ...
      const content = await control.request(JSON.parse(request)).catch(e => reject(e));
      if ('groupList' in content) resolve(content.groupList);else reject(new Error('  groupList not found.'));
    });
  }
  /**
  ...
  async function listWorkspaces() {
    return new Promise(async (resolve, reject) => {
      ...
      const content = await control.request(JSON.parse(request)).catch(e => reject(e));
      if ('workspaceList' in content) resolve(content.workspaceList);else reject(new Error('workspaceList not found.'));
    });
  }
  /**
  ...
  async function getDefaultWorkspace() {
    return new Promise(async (resolve, reject) => {
      ...
      const content = await control.request(JSON.parse(request)).catch(e => reject(e));
      if ('workspace' in content) resolve(content.workspace);else reject(new Error('No default workspace found.'));
    });
  ...
  async function listFunctions() {
    return new Promise(async (resolve, reject) => {
      ...
      if ('functionList' in content) resolve(content);else reject(new Error('functionList not found.'));
    });
  }
  /**
   *Queries the corelink relay for available functions
  ...
  async function describeFunction(options) {
    return new Promise(async (resolve, reject) => {
      ...
      request.token = token;
      const content = await control.request(request).catch(e => reject(e));
      if ('description' in content) resolve(content);else reject(new Error('description not found.'));
    });
  }
  /**
   *Queries a specific function to get its self descriptor
  ...
  */
  ...
  async function subscribe(options) {
    return new Promise(async (resolve, reject) => {
      let workOptions;
      if (typeof options === 'undefined') workOptions = {};else workOptions = options;
      if (!('streamIDs' in workOptions)) workOptions.streamIDs = [];
  ...
  */
  ...
  async function generic(options) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!('function' in workOptions)) reject(new Error('No function name')); // adding token to request
      ...
      if (content) resolve(content);else reject(new Error('Wrong Expresions.'));
    });
  }
  /**
  ...
  */
  ...
  async function exit() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const dis = await disconnect({
        streamIDs
      }).catch(err => reject(err));
      if (dis === true) resolve(true);else reject(dis);
    });
