...
export function init() {
  ...
  window.EventBus.subscribe("initialize", json => {
    const id = json["id"];
    initSelfAvatar(id); // create other avatars joined earlier
  ...
  window.EventBus.subscribe("join", json => {
    console.log("receive event: join"); // console.log(json);
    ...
    console.log("join window.avatars", window.avatars); // window.updatePlayersMenu();
  });
  window.EventBus.subscribe("leave", json => {
    ...
    if (window.avatars[json["user"]]) {
      ...
      window.avatars[json["user"]].leave = true; // TODO clean up when too many
  ...
  window.EventBus.subscribe("avatar", json => {
    ...
    if (payload["user"] in window.avatars) {
      ...
      if (window.avatars[payload["user"]] && "leave" in window.avatars[payload["user"]]) {
  ...
  window.EventBus.subscribe("webrtc", json => {
    ...
    console.log("receive webrtc", json, signal, json["ts"], Date.now()); // if (!signal.roomID)
    //     return;

    var peerUuid = signal.uuid; // Ignore messages that are not for us or from ourselves

    if (peerUuid == window.localUuid || signal.dest != window.localUuid && signal.dest != "all" // && signal.roomID != window.avatars[window.playerid].roomID
    ) return;

    if (signal.displayName != undefined && signal.dest == "all" // && signal.roomID == window.avatars[window.playerid].roomID
    ) {
      // set up peer connection object for a newcomer peer
      console.log("[webrtc] case 1: set up peer connection object for a newcomer peer:" + peerUuid);
      window.setUpPeer(peerUuid, signal.displayName); // corelink
      ...
      console.log("[webrtc] corelink.send from", window.localUuid, "to", peerUuid, msg); // JSON.stringify({ 'MR_Message': "Broadcast_All", 'displayName': MRVoip.username, 'uuid': MRVoip.localUuid, 'dest': peerUuid, 'roomID': MRVoip.roomID }));
    } else if (signal.displayName != undefined && signal.dest == window.localUuid // && signal.roomID == window.avatars[window.playerid].roomID
    ) {
      // initiate call if we are the newcomer peer
      console.log("[webrtc] case 2: initiate call if we are the newcomer peer:" + peerUuid);
  ...
  window.EventBus.subscribe("object", json => {
    ...
    let dirtyObject = json["state"]; // { type: objectType, matrix: objectMatrix, objid: objid }

    if (dirtyObject["type"] in window.models) {
      if (!(dirtyObject["objid"] in window.objects)) window.objects[dirtyObject["objid"]] = new SyncObject(dirtyObject["objid"], dirtyObject["type"]);
  ...
  window.EventBus.subscribe("event", json => {
    window.eventOwner = json["uid"]; // console.log("window.EventBus.subscribe('event'", json["state"], json);
