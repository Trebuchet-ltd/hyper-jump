...
const run = async () => {
  ...
  config.host = 'corelink.hpc.nyu.edu'; // config.host = 'localhost'
  ...
  if (await corelink.connect({
    ...
    corelink.on('dropped', streamID => {
      ...
      var iDiv = document.getElementById(streamID.streamID);
      var btnid = "stream" + streamID.streamID;
      console.log(btnid); // var btn = document.getElementById(btnid)
    ...
    if (metaroomSyncSender = await corelink.createSender({
      ...
      console.log("ZH: metaroomSyncSender", metaroomSyncSender); // initialize
    ...
    if (metaroomWebrtcSender = await corelink.createSender({
      ...
      console.log("ZH: metaroomWebrtcSender", metaroomWebrtcSender); // start webrtc signalling
      // window.webrtc_start();
    }
    ...
    if (metaroomInitSender = await corelink.createSender({
      ...
      if (metaroomReceiver != "") {
        ...
        corelink.send(metaroomInitSender, msg); // setTimeout(10000, corelink.send(metaroomInitSender, msg));
    ...
    metaroomReceiver.forEach(async data => {// console.log("[webrtc debug] metaroomReceiver.forEach", data.streamID, data.meta.name)
    ...
    corelink.on('data', (streamID, data, header) => {
      ...
      window.EventBus.publish(receiverdata["type"], receiverdata); // if (receiverdata["type"] != "avatar" && receiverdata["type"] != "realsense")
