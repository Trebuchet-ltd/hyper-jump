...
window.currentName = '';
window.currentID = -1; // scene state
  ...
  if (currentDemo != null) {
    ...
    loadScene(currentDemo).then(() => {
      ...
      if (currentDemo.world.onReload) {
        ...
        currentDemo.world.onReload(clay.model, currentDemo.ctx, currentDemo.ctxForever).then(() => {
        ...
        currentDemo.ctx = {};
        currentDemo.isValid = false;
        onReloadDefault(currentDemo.world, clay.model, currentDemo.ctx, currentDemo.ctxForever).then(() => {
          currentDemo.isValid = prevIsValid;
        }).catch(err => {
          console.error(err.message, err.stack);
...
const addDemoButtons = demoNames => {
  ...
  for (let n = 0; n < names.length; n++) {
    ...
    window["getSceneFlagByID"]["" + n] = function () {
      return window[flag];
    };

    window["setSceneFlagByID"]["" + n] = function (f) {
      window[flag] = f;
    };

    if (names[n] != "Speak") {
      header.innerHTML += '<button onclick=chooseFlag("' + names[n] + '");' + 'window.syncDemos();>' + names[n] + '</button>';
      clay.vrWidgets.add('label').info(names[n]).move(1, 1.7 - n * .1, .7).turnY(Math.PI / 6).scale(.045);
    } else {
      header.innerHTML += '<button id=\"Speak\" onclick=\"window.' + flag + '=!window.' + flag + ';window.muteSelf()\">' + names[n] + '</button>';
    }
  }

  lcb = new ControllerBeam(clay.vrWidgets, 'left');
  rcb = new ControllerBeam(clay.vrWidgets, 'right');
  header.innerHTML += "<br>"; // NOTE(KTR): for code-reloading during development / for permanent changes
  ...
  if (enableSceneReloading) {
    // use a button
    header.innerHTML += '<button onclick=reloadCurrentScene();>Reload</button><BR>'; // or automatically reload if you exit and re-enter the window

    window.addEventListener("focus", event => {
      window.reloadCurrentScene();
    }, false); // or eventually the server should just tell the application when a file has changed
    // TODO: 
    // ...
  }
...
function runDemo(demo) {
  if (!demo._isStarted) {
    currentDemo = demo; // default : remove all the previous demos when starting a new one
    ...
    if (!demo.world.init) {
      ...
      if (!demo.world.display) {// console.warn("no display function");
      }
    }
  ...
  if (demo._isReady && demo.world.display) {
    try {
      ...
      console.error(err.message, err.stack);
      console.error("client code has runtime errors in display()!"); //window.chooseFlag(window.currentName);
    }
  }
}
