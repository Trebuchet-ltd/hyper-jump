...
export const init = async model => {
  ...
  let N = 100; // USE A MONTE CARLO METHOD TO GENERATE UNIFORMLY RANDOM DOTS ON A SPHERE
  ...
  let r = Math.sqrt(4 * Math.PI / N); // CREATE THE DOTS GEOMETRY

  model.move(0, 1.5, 0).scale(.5);

  for (let n = 0; n < N; n++) model.add('tubeZ').color(C[n]); // AT EACH ANIMATION FRAME


  model.animate(() => {
    let rr = r * r; // REPEL ALL THE DOTS FROM EACH OTHER
    ...
    for (let i = 0; i < N; i++) for (let j = 0; j < N; j++) if (j != i) {
      ...
      let dd = cg.dot(d, d);
      if (dd < rr / 4) P[j] = generatePoint();else if (dd < rr) {
    ...
    let s = [Math.sin(model.time), Math.sin(model.time + Math.PI / 3), Math.sin(model.time + Math.PI / 3 * 2)];

    for (let n = 0; n < N; n++) {
      for (let j = 0; j < 3; j++) P[n][j] += s[j] * (2 * C[n][j] - 1) * model.deltaTime / 2;

      P[n] = cg.normalize(P[n]);
    } // RENDER EACH DOT AS A FLAT DISK ORIENTED TOWARD THE SPHERE SURFACE


    for (let n = 0; n < N; n++) model.child(n).identity().move(P[n]).aimZ(P[n]).scale(r / 4, r / 4, .0001);
  });
};