...
for (let i = 0; i < buttonNum; i++) buttonState.left[i] = buttonState.right[i] = {
  pressed: false,
  touched: false,
  value: 0
};
...
export function ControllerBeam(model, hand) {
  ...
  let bend = Math.PI / 4;
  this.beam = model.add();
  this.beam.add('tubeZ').color(10, 0, 0).turnX(-bend).move(0, 0, -10.01).scale(.001, .001, 10);

  this.update = matrix => {
    let m = matrix ? matrix : controllerMatrix[hand],
        update = (offset, fallback) => this.beam.setMatrix(this.m = m.length ? cg.mMultiply(m, cg.mTranslate(offset)) : cg.mTranslate(fallback));
  ...
  this.hitLabel = label => this.hitRect(cg.mMultiply(label.getGlobalMatrix(), cg.mScale(label.getInfo().length / 2, 1, 1)));
}