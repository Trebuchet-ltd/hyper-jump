...
const util = require("util"); // const yargs = require('yargs');
// based on examples at https://www.npmjs.com/package/ws


const WebSocketServer = WebSocket.Server;
var env = "remote";
var port = 8447; // process console parameters
...
httpsServer.listen(HTTPS_PORT); // console.log("httpsServer:", httpsServer);
// ----------------------------------------------------------------------------------------
// Create a server for handling websocket calls

const wss = // argv.env == "local" ? new WebSocketServer({ port: 3448 }) :
new WebSocketServer({
  server: httpsServer
...
setInterval(() => {
  ...
  console.log(Array.from(websocketMap.keys())); // console.log("avatars: ");
  // console.log(avatars);
...
function leave(index, username) {
  ...
  console.log(avatars); // clearInterval(timerID);
...
wss.on("connection", function (ws, req) {
  ...
  send(ws.index, -1, payload2others); // send('*', ws.index, payload2self);
  ...
  ws.on("message", function (data) {
    ...
    switch (json["type"]) {
      ...
      case "objectInit":
        {
          ...
          if (state["objid"] == 0) {
            ...
            if (datastore.exists(state["objid"])) {// already create, then ignore this request
            } else {
      ...
      case "webrtc":
        {
          console.log("receive ws msg:", json["type"], json["ts"], Date.now()); // console.log(json["type"], avatars);

          const key = json["uid"];
          const state = json["state"]; // avatars[key].webrtc = state[uuid];
      ...
      case "mute":
        {
          console.log("receive mute from ", ws.index, json["state"], "at", Date.now());
          const response = {
            type: "mute",
            id: ws.index,
            ts: Date.now(),
